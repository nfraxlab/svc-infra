...
def run_test():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"[OK] Created dummy file: {user_model}")
        ...
        if exists:
            print("[OK] PASS: Detected existing model:")
            for f in files:
                print(f"   - {f}")
        else:
            print("[X] FAIL: Did not detect existing model")
            return 1
        ...
        if not exists:
            print("[OK] PASS: Correctly identified missing model")
        else:
            print(f"[X] FAIL: Incorrectly detected model exists: {files}")
            return 1
        ...
        if exists and len(files) == 1:
            print("[OK] PASS: Detected partial existence:")
            print(f"   - {files[0]}")
        else:
            print("[X] FAIL: Did not correctly detect partial existence")
            return 1
